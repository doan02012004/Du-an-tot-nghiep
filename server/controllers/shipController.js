...
export const updateWeightRate = async (req, res) => {
    ...
    try {
        ...
        if (!ship) {
            return res.status(404).json({ message: 'Không tìm thấy ship này' });
        }
        ...
        res.status(200).json({
            ...
            updatedWeightRate: weightRate
        });
    } catch (err) {
        console.error(err); // Log lỗi chi tiết
        res.status(500).json({ message: 'Có lỗi xảy ra khi cập nhật khối lượng', error: err.message });
    }
};
...
export const updateVolumeRate = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({
            ...
            updatedVolumeRate: volumeRate
        });

    } catch (err) {
        console.error(err); // Log lỗi chi tiết
        res.status(500).json({ message: 'Có lỗi xảy ra khi cập nhật thể tích', error: err.message });
    }
};
