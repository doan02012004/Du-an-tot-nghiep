...
const ColorItem = ({ data }: ColorItemProps) => {
  ...
  const onEdit = () => {
    ...
    const newGallers = gallerys.map((item: Igallery) =>
      item.name == data.name ? newGallery : item
    );
    ...
    <div className="w-full mb-4 bg-white border rounded-lg shadow-sm shadow-gray-500">
      ...
      <div className="flex items-center justify-between p-3 border-b cursor-pointer">
        ...
        <div className="flex items-center gap-x-3">
          {data.check == false ? (
            <Button
              disabled={loadingAvt ? loadingAvt : loadingGal}
              onClick={onSave}
              type="primary"
            >
              <SaveOutlined />
            ...
            <Button onClick={onEdit} className="text-white bg-yellow">
              <EditOutlined />
            </Button>
          )}
          <Button className="rounded-full size-8" onClick={onOpen}>
            <DownOutlined />
          </Button>
        </div>
      </div>
